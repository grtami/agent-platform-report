        ...
        markArea: {
          ...
          data: [
            [{
              ...
              xAxis: 7,
              yAxis: 8,
              itemStyle: { color: 'rgba(168,85,247,0.06)' },
              label: { color: '#a855f7', fontSize: 11, opacity: 0.6 }
            }, { xAxis: 10, yAxis: 10 }],
            [{
              name: '低代码友好型',
              xAxis: 8,
              yAxis: 4,
              itemStyle: { color: 'rgba(6,182,212,0.06)' },
              label: { color: '#06b6d4', fontSize: 11, opacity: 0.6 }
            }, { xAxis: 10, yAxis: 7 }],
            [{
              name: '工程化企业型',
              xAxis: 4,
              ...
              label: { color: '#6366f1', fontSize: 11, opacity: 0.6 }
            }, { xAxis: 7, yAxis: 10 }]
          ]
        }
      }]
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    loadData().catch(err => {
      ...
      document.body.innerHTML = `<div style="padding:40px;color:#f87171;text-align:center;">
        <h2>数据加载失败</h2><p>请确保 data.json 与 index.html 在同一目录，并通过 HTTP 服务访问（如 GitHub Pages）。</p>
        <p style="color:#94a3b8;margin-top:12px;">${err.message}</p>
      </div>`;
    });
  });
})();
